...
describe("assertNoViolations", () => {
  it("throws only when something is wrong, and carries every violation", () => {
    ...
    const found = [{ rule: "task.assignmentId", path: "task.assignmentId", message: "needed" }];
...
describe("validateTask", () => {
  ...
  it("lets a control stand locked in its place, naming the level, and never a queue's own content", () => {
    ...
    expect(caps({ conference: { lockedBy: "org" } })).toEqual([]);
    ...
    expect(caps({ conference: { destinations: [{ id: "t2", label: "Tier 2" }] } })).toEqual([]);
    expect(caps({ conference: { lockedBy: "org" } })).toEqual([]);
    // A follow-up on the calendar is a flag control, like hold: present, or locked in its place.
  ...
  it("states what the record adds up to before this agent, each total present when known", () => {
    const record = (over: Record<string, unknown>) => rules(validateTask(task({ history: { steps: [{ step: "answered", at: "2026-08-21T00:59:41Z", by: "a-17" }], ...over } }), { channel: "voice" }));
    ...
    expect(record({ holdSeconds: -5 })).toEqual(["task.history.holdSeconds"]);
    expect(record({ queueSeconds: 1.5 })).toEqual(["task.history.queueSeconds"]);
    expect(record({ interactionSeconds: "312" })).toEqual(["task.history.interactionSeconds"]);
    expect(record({ transfers: -1 })).toEqual(["task.history.transfers"]);
    expect(rules(validateTask(task({ history: { transfers: 2 } }), { channel: "voice" }))).toEqual(["task.history.steps.shape"]);
    expect(rules(validateTask(task({ history: [] }), { channel: "voice" }))).toEqual(["task.history.shape"]);
    // The entries moved under steps, and a violation says where it found them.
    const misstep = validateTask(task({ history: { steps: [{ step: "ringing", at: "2026-08-21T09:00:00Z" }] } }), { channel: "voice" });
    expect(misstep.map(v => `${v.rule} @ ${v.path}`)).toEqual(["task.history.step @ task.history.steps[0].step"]);
  });

  it("records each hold as its own entry, oldest first", () => {
    const history = (steps: unknown) => rules(validateTask(task({ history: { steps } }), { channel: "voice" }));
    ...
    const paused = (steps: unknown) => rules(validateTask(task({ phase: "paused", history: { steps } }), { channel: "voice" }));
    ...
    expect(history([answered, { step: "muted", at: "2026-08-21T01:00:00Z", seconds: 4, mutedBy: "host" }])).toEqual(["task.history.muted.by"]);
    ...
    expect(history([answered, { step: "muted", at: "2026-08-21T01:00:00Z", seconds: 4, by: "a-17" }])).toEqual(["task.history.mutedBy"]);
    expect(history([answered, { step: "muted", at: "2026-08-21T01:00:00Z", seconds: 4, by: "a-17", mutedBy: "headset" }])).toEqual(["task.history.mutedBy"]);
    expect(history([answered, { step: "held", at: "2026-08-21T01:00:00Z", seconds: 4, mutedBy: "host" }])).toEqual(["task.history.mutedBy.unexpected"]);
    // Oldest first: a hold filed before the answer it followed is out of its turn.
    expect(history([{ step: "held", at: "2026-08-21T01:02:10Z", seconds: 35, by: "a-17" }, answered])).toEqual(["task.history.order"]);
    ...
    expect(paused([answered, { step: "held", at: "soon", by: "a-17" }, { step: "held", at: "2026-08-21T01:06:48Z", by: "a-17" }])).toEqual(["task.history.at"]);
  });
  ...
  it.each([
    ...
    ["an empty assignment id", { assignmentId: "  " }, "task.assignmentId"],
  ...
  it("requires a task to name this life of itself, the assignment minted per offer", () => {
    expect(rules(validateTask(task(), { channel: "voice" }))).toEqual([]);
    expect(rules(validateTask(task({ assignmentId: undefined }), { channel: "voice" }))).toEqual(["task.assignmentId"]);
    expect(rules(validateTask(task({ assignmentId: "" }), { channel: "voice" }))).toEqual(["task.assignmentId"]);
  });
  ...
  it("omits an unfinished duration rather than reporting nought", () => {
    const history = (over: Record<string, unknown>) =>
      check({ history: { steps: [{ step: "answered", at: "2026-08-21T09:00:00Z", ...over }] } });
    expect(history({ seconds: 0 })).toContain("task.history.seconds");
    expect(history({ seconds: -5 })).toContain("task.history.seconds");
    expect(history({ seconds: 22 })).toEqual([]);
    expect(history({})).toEqual([]);
  });

  it("takes a history step without a person, and rejects an empty one", () => {
    const history = (over: Record<string, unknown>) =>
      check({ history: { steps: [{ step: "answered", at: "2026-08-21T09:00:00Z", ...over }] } });
    // Absent means the provider could not attribute it, which is a legitimate report.
    expect(history({})).toEqual([]);
    expect(history({ by: "" })).toContain("task.history.by");
    expect(history({ by: "agent-17" })).toEqual([]);
    expect(check({ history: { steps: [{ step: "ringing", at: "2026-08-21T09:00:00Z" }] } })).toContain("task.history.step");
  });
...
describe("break state", () => {
  ...
  it("refuses accepting beside canRequestBreak, which replaced it", () => {
    // A rename is a refusal, not an alias: an adapter still speaking the old word is told so.
    const brk = (state: Record<string, unknown>) => rules(validateSnapshot(snapshot({ break: { status: "not-requested", ...state } }), manifest()));
  ...
  it("accepts each approval and rejects one the contract dropped", () => {
    ...
    expect(rules(validateSnapshot(snapshot({ break: { status: "starting-after-task", canRequestBreak: true }, tasks: [task()] }), manifest()))).toEqual([]);
    expect(check({ status: "starting-after-task" })).toEqual(["break.starting-after-task.tasks"]);
    {
    }
    ...
    expect(check({ status: "approved" })).toContain("break.status");
    expect(check({ status: "denied" })).toContain("break.status");
  });
  ...
  it("refuses a break in effect beside a task, and accepts one that is waiting for it to end", () => {
    ...
    // stand beside any task; on-break beside one is a report of a state the agent cannot be in.
    const withTask = (status: string) =>
      rules(validateSnapshot(snapshot({ tasks: [task()], break: { status, canRequestBreak: true } }), manifest()));
    expect(withTask("starting-after-task")).toEqual([]);
    expect(withTask("granted")).toEqual([]);
    expect(withTask("on-break")).toEqual(["break.on-break.tasks"]);
    expect(check({ status: "on-break" })).toEqual([]);
  });

  it("reports an active reason only when a break is happening", () => {
    expect(check({ status: "not-requested", activeReasonId: "lunch" })).toContain("break.activeReasonId.status");
    expect(check({ status: "on-break", activeReasonId: "lunch" })).toEqual([]);
    expect(check({ status: "on-break", activeReasonId: "" })).toContain("break.activeReasonId");
  });
  ...
  it("requires a break in effect on a provider that publishes reasons to name the one it is on, a forced one included", () => {
    ...
    // The platform itself forces a break on its own rule or a schedule: provider, and no person named.
    expect(check({ status: "on-break", forced: { by: "provider" } })).toEqual([]);
    expect(check({ status: "on-break", forced: { by: "lead-3" } })).toEqual([]);
    expect(check({ status: "on-break", forced: { by: "" } })).toEqual(["break.forced.by"]);
  ...
  it("keeps the actor and accepts only advisory forced-break duration", () => {
    ...
    // How long is left, as seconds from this publication, so every screen counts down the same number.
    expect(forced({ by: "lead-3", expectedEndsInSeconds: 600 })).toEqual([]);
    expect(forced({ by: "lead-3", expectedEndsInSeconds: 0 })).toEqual([]);
    expect(forced({})).toContain("break.forced.by");
    expect(forced({ by: "" })).toContain("break.forced.by");
    for (const expectedEndsInSeconds of [-1, 1.5, Infinity, NaN, "500", null]) {
      expect(forced({ by: "lead-3", expectedEndsInSeconds })).toContain("break.forced.expectedEndsInSeconds");
    }
    // The duration asked for travels on the lead's command; the state says what is left.
    expect(forced({ by: "lead-3", expectedDurationMs: 600000 })).toEqual(["break.forced.expectedDurationMs.renamed"]);
...
describe("validateTeamMembers", () => {
  ...
  it("carries the team's policies on the team member list, as the lead sees them", () => {
    ...
    expect(rules(validateTeamMembers({ members: [] }, "team", may))).toEqual([]);
    expect(rules(validateTeamMembers({ members: [], policies: {} }, "team", may))).toEqual([]);
  ...
  it("accepts a conforming team member list", () => {
    ...
    expect(validateTeamMembers({ members: [{ id: "A-2", availability: "elsewhere", since: "2026-08-21T09:00:00Z" }] })).toEqual([]);
    // Nobody asking is absence on each member: the list carries no request of its own, and the former one is not read.
    expect(validateTeamMembers({ members: [] })).toEqual([]);
    expect(validateTeamMembers({ members: [], requests: [{ memberId: "A-2", assignmentId: "" }] })).toEqual([]);
  });
  ...
  it("carries each member's open tasks, trimmed by the provider, and the lead's own listening", () => {
    ...
    const withTasks = (tasks: unknown, over: Record<string, unknown> = {}) => rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", tasks, ...over }] }));
    ...
    const listening = (over: Record<string, unknown>, tasks?: unknown[]) => rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", ...(tasks === undefined ? {} : { tasks }), listening: [entry(over)] }] }));
    for (const mode of ["listen", "coach"]) expect(listening({ mode })).toEqual([]);
    ...
    expect(listening({ leadId: "" })).toEqual(["team.member.listening.leadId"]);
    ...
    expect(rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", listening: entry() }] }))).toEqual(["team.member.listening.shape"]);
    expect(rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", listening: [] }] }))).toEqual(["team.member.listening.empty"]);
    expect(rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", listening: ["L-9"] }] }))).toEqual(["team.member.listening.entry"]);
    // Two leads on one member's call is ordinary; the same lead twice is not.
    expect(rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", listening: [entry(), entry({ leadId: "L-4", mode: "coach" })] }] }))).toEqual([]);
    expect(rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", listening: [entry(), entry({ mode: "coach" })] }] }))).toEqual(["team.member.listening.unique"]);
    expect(rules(validateTeamMembers({ members: [
      { id: "A-2", availability: "on-task", listening: [entry()] },
      { id: "A-3", availability: "on-task", listening: [entry({ assignmentId: "alloc-8", mode: "coach" })] },
    ] }))).toEqual(["team.listening.single"]);
    expect(rules(validateTeamMembers({ members: [
      { id: "A-2", availability: "on-task", listening: [entry()] },
      { id: "A-3", availability: "on-task", listening: [entry({ leadId: "L-4", assignmentId: "alloc-8", mode: "coach" })] },
    ] }))).toEqual([]);
    // A lead who is heard is in the room the member sees; a silent one leaves no trace there.
    const room = (onCall: unknown[]) => ({ ...trimmed, onCall });
    const party = { role: "party", since: at };
    expect(listening({ mode: "join-call" }, [room([party, { role: "agent", userId: "A-2", since: at }, { role: "agent", userId: "L-9", since: at }])])).toEqual([]);
    expect(listening({ mode: "join-call" }, [room([party, { role: "agent", userId: "A-2", since: at }])])).toEqual(["team.member.listening.heard"]);
    expect(listening({ mode: "listen" }, [room([party, { role: "agent", userId: "A-2", since: at }])])).toEqual([]);
  });
  ...
  it("carries the agent's own day on their snapshot and on shift-updated, in the shape their lead sees", () => {
    ...
    const ticking = manifest({ timeCheck: true });
    const read = (over: Record<string, unknown>) => snapshot({ providerTime: at, ...over });
    expect(rules(validateSnapshot(read({ shift: day }), ticking))).toEqual([]);
    expect(rules(validateSnapshot(read({ shift: { talkSeconds: 5 } }), ticking))).toEqual(["shift.signedInAt"]);
    expect(rules(validateSnapshot(read({ shift: "today" }), ticking))).toEqual(["shift.shape"]);
    const check = (event: unknown) => rules(validateEventEnvelope(envelope(event), ticking));
    expect(check({ type: "shift-updated", shift: day })).toEqual([]);
    expect(check({ type: "shift-updated", shift: { ...day, tasksHandled: 2.5 } })).toEqual(["shift.tasksHandled"]);
    // The member's copy on the lead's list is the same shape under its own rule prefix.
    expect(rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", shift: { ...day, tasksHandled: 2.5 } }] }))).toEqual(["team.member.shift.tasksHandled"]);
  });

  it("counts every duration off the active call from the provider's clock: timeCheck where a running instant is published, providerTime with it", () => {
    const at = "2026-08-21T09:00:00Z";
    const ticking = manifest({ timeCheck: true });
    // A snapshot from a provider with a clock states it; one without a clock states none.
    expect(rules(validateSnapshot(snapshot({ providerTime: at }), ticking))).toEqual([]);
    expect(rules(validateSnapshot(snapshot(), ticking))).toEqual(["snapshot.providerTime"]);
    expect(rules(validateSnapshot(snapshot({ providerTime: "now" }), ticking))).toEqual(["snapshot.providerTime"]);
    expect(rules(validateSnapshot(snapshot({ providerTime: at }), manifest()))).toEqual(["snapshot.providerTime.unexpected"]);
    // A running instant -- a member's since, a lead's listening.since, a signedInAt, an onCall.since -- needs the clock it is counted by.
    const lead = { capabilities: { lead: true as const }, leadFeatures: true };
    const member = (over: Record<string, unknown>) => ({ members: [{ id: "A-2", availability: "on-task", ...over }] });
    expect(rules(validateSnapshot(snapshot({ providerTime: at, team: member({ since: at }) }), ticking, "snapshot", lead))).toEqual([]);
    expect(rules(validateSnapshot(snapshot({ team: member({ since: at }) }), manifest(), "snapshot", lead))).toEqual(["manifest.timeCheck.required"]);
    expect(rules(validateSnapshot(snapshot({ team: member({}) }), manifest(), "snapshot", lead))).toEqual([]);
    expect(rules(validateSnapshot(snapshot({ shift: { signedInAt: at } }), manifest()))).toEqual(["manifest.timeCheck.required"]);
    expect(rules(validateSnapshot(snapshot({ tasks: [task({ onCall: [{ role: "party", since: at }] })] }), manifest()))).toEqual(["manifest.timeCheck.required"]);
    expect(rules(validateSnapshot(snapshot({ tasks: [task({ onCall: [{ role: "party", since: at }] })], providerTime: at }), ticking))).toEqual([]);
    const on = (event: unknown, m: unknown) => rules(validateEventEnvelope(envelope(event), m, "event", lead as never));
    expect(on({ type: "team-member-updated", member: { id: "A-2", availability: "on-task", listening: [{ leadId: "L-9", assignmentId: "alloc-7", mode: "listen", since: at }] } }, manifest())).toEqual(["manifest.timeCheck.required"]);
    expect(on({ type: "team-member-updated", member: { id: "A-2", availability: "on-task", listening: [{ leadId: "L-9", assignmentId: "alloc-7", mode: "listen", since: at }] } }, ticking)).toEqual([]);
    expect(on({ type: "shift-updated", shift: { signedInAt: at } }, manifest())).toEqual(["manifest.timeCheck.required"]);
    expect(rules(validateEventEnvelope(envelope({ type: "snapshot", reason: "reconnected", snapshot: snapshot({ shift: { signedInAt: at } }) }), manifest()))).toEqual(["manifest.timeCheck.required"]);
  });

  it("carries each member's own shift: sign-in, sign-out, the day's totals and its events", () => {
    const at = "2026-08-21T09:00:00Z";
    const shift = (over: Record<string, unknown>) => rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", shift: { signedInAt: at, ...over } }] }));
    ...
    expect(rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", shift: { talkSeconds: 5 } }] }))).toEqual(["team.member.shift.signedInAt"]);
    ...
    expect(rules(validateTeamMembers({ members: [{ id: "A-2", availability: "on-task", shift: "today" }] }))).toEqual(["team.member.shift.shape"]);
  });

  it("rejects the agent it is published to, once told who that is", () => {
    // A lead does not report to themself. The team member list below carries a colleague and the
    // reader, so the check has to pick the reader out rather than object to the list.
    const published = {
    ...
    expect(found("1042")).toEqual(["team.member.self at team.members[1].id"]);
    // One member alone, as team-member-updated carries them, is held to the same reader.
    expect(rules(validateTeamMember({ id: "1042", availability: "on-task" }, "member", { self: "1042" }))).toEqual(["team.member.self"]);
    expect(rules(validateTeamMember({ id: "A-2", availability: "on-task" }, "member", { self: "1042" }))).toEqual([]);
...
describe("validateSnapshot", () => {
  ...
  it.each([
    ...
    ["two tasks with one id", snapshot({ tasks: [task(), task()] }), "task.assignmentId.unique"],
  ])("rejects %s", (_label, value, rule) => {
  ...
  it("holds the team member list to the login once told what it declares", () => {
    // The login is the permission, and it cuts both ways: only a lead's snapshot may carry a team
    // member list, and once the lead has switched the feature on it must. Both agreeing cases
    // pass, so each refusal is about the disagreement.
    ...
    // The flag alone owes nothing: the provider assumes nothing until the application says on.
    expect(rules(validateSnapshot(snapshot(), manifest(), "snapshot", lead))).toEqual([]);
...
describe("validateEventEnvelope", () => {
  ...
  it("carries a diagnostic as two sentences and, where there is one, a task", () => {
    ...
    expect(check({ ...diagnostic, assignmentId: "alloc-77" })).toEqual([]);
    // A task is named by its assignment alone; the retired name is refused beside it.
    expect(check({ ...diagnostic, assignmentId: "" })).toEqual(["event.diagnostic.assignmentId"]);
    expect(check({ ...diagnostic, assignmentId: "alloc-77", taskId: "call-77" })).toEqual(["event.assignmentId.renamed"]);
  ...
  it("holds a reconnect snapshot to the login as it holds the first one", () => {
    // "On every snapshot" includes the one a reconnect carries, once the feature is on.
    const team = { members: [{ id: "A-2", availability: "ready" }] };
    const reconnect = (snap: unknown, context: unknown) =>
      rules(validateEventEnvelope(envelope({ type: "snapshot", reason: "reconnected", snapshot: snap }), manifest(), "event", context as never));
    const on = { capabilities: { lead: true as const }, leadFeatures: true };
    expect(reconnect(snapshot({ team }), on)).toEqual([]);
    expect(reconnect(snapshot(), { capabilities: {} })).toEqual([]);
    expect(reconnect(snapshot(), on)).toEqual(["team.required"]);
    expect(reconnect(snapshot({ team }), { capabilities: {} })).toEqual(["team.unentitled"]);
  });
  ...
  it("validates each event type", () => {
    ...
    // One member at a time after the whole team: the member whole, a member taken off, the policies.
    expect(check({ type: "team-member-updated", member: { id: "A-2", availability: "on-task" } })).toEqual([]);
    expect(check({ type: "team-member-updated", member: { id: "", availability: "on-task" } })).toEqual(["team.member.id"]);
    expect(check({ type: "team-member-updated", member: "A-2" })).toEqual(["team.member.shape"]);
    expect(check({ type: "team-member-removed", memberId: "A-2" })).toEqual([]);
    expect(check({ type: "team-member-removed", memberId: "" })).toEqual(["event.teamMemberRemoved.memberId"]);
    expect(check({ type: "team-policies-updated", policies: { hold: { setting: "off", setBy: "team" } } })).toEqual([]);
    expect(check({ type: "team-policies-updated", policies: { hold: { setting: "sometimes", setBy: "team" } } })).toEqual(["team.policy.setting"]);
  ...
  it("validates every task outcome", () => {
    ...
    // renamed away: a transfer is not something the agent does, so no task ends transferred.
    expect(ended({ type: "transferred", destinationId: "tier2" })).toEqual(["event.taskEnded.outcome.type"]);
...
describe("validateHistoryReport", () => {
  it("takes a leg the host performed as it begins, runs, and ends", () => {
    const report = (over: Record<string, unknown> = {}) => rules(validateHistoryReport({ assignmentId: "alloc-42", step: "muted", at: "2026-08-21T09:00:00Z", mutedBy: "host", ...over }));
    expect(report()).toEqual([]);
    // A muted leg says whose the silence was; no other leg has anyone to name for it.
    expect(report({ mutedBy: "station" })).toEqual([]);
    expect(report({ mutedBy: undefined })).toEqual(["historyReport.mutedBy"]);
    ...
    expect(report({ ended: true })).toEqual(["historyReport.ended.seconds"]);
    expect(report({ ended: false })).toEqual(["historyReport.ended"]);
    expect(report({ seconds: 0 })).toEqual(["historyReport.seconds"]);
    expect(report({ assignmentId: "" })).toEqual(["historyReport.assignmentId"]);
    expect(report({ taskId: "call-42" })).toEqual(["historyReport.assignmentId.renamed"]);
    expect(report({ step: "whispered" })).toEqual(["historyReport.step"]);
    expect(report({ at: "now" })).toEqual(["historyReport.at"]);
    expect(rules(validateHistoryReport("muted"))).toEqual(["historyReport.shape"]);
    // What a provider never asked for never crosses: a running report reaches only a manifest that declares it.
    const running = { assignmentId: "alloc-42", step: "muted", at: "2026-08-21T09:00:00Z", mutedBy: "host", seconds: 15 };
    expect(rules(validateHistoryReport(running, "report", manifest({ runningStepReports: true })))).toEqual([]);
    expect(rules(validateHistoryReport(running, "report", manifest()))).toEqual(["historyReport.running.unexpected"]);
    expect(rules(validateHistoryReport({ ...running, seconds: 42, ended: true }, "report", manifest()))).toEqual([]);
    expect(rules(validateHistoryReport({ assignmentId: "alloc-42", step: "muted", at: "2026-08-21T09:00:00Z", mutedBy: "host" }, "report", manifest()))).toEqual([]);
...
describe("validateResult", () => {
  ...
  it("lets a provider name its own codes and holds the omni namespace to the contract", () => {
    ...
    const ended = (failure: unknown) => rules(validateEventEnvelope(envelope({ type: "task-ended", assignmentId: "alloc-42", outcome: { type: "failed", failure } }), manifest()));
...
describe("the other direction, everywhere", () => {
  ...
  it("gives a directory something to offer, and keeps its ids unique", () => {
    const directory = (over: unknown) => rules(validateTask(task({ capabilities: { conference: over } }), voice));
  ...
  it("gives a required outcome to the agent to complete, and a wrap to collect it in", () => {
    const codes = [{ id: "resolved", label: "Resolved" }];
    const under = (over: Record<string, unknown>) => rules(validateTask(task({ capabilities: { outcomes: { required: true, codes } }, ...over }), voice));
    // A required code is the agent's to give, so the task waits for the agent's complete.
    expect(under({ completionMode: "agent-command" })).toEqual([]);
  ...
  it("names nobody on a queued step", () => {
    const step = (entry: Record<string, unknown>) => rules(validateTask(task({ history: { steps: [{ at: "2026-08-21T09:00:00Z", ...entry }] } }), voice));
    expect(step({ step: "answered", by: "A-1" })).toEqual([]);
    expect(step({ step: "queued" })).toEqual([]);
    expect(step({ step: "queued", by: "A-1" })).toEqual(["task.history.by.unexpected"]);
  });
  ...
  it("holds the break state's parts to its approval", () => {
    ...
    expect(check({ status: "on-break", reasons, activeReasonId: "lunch" })).toEqual([]);
    expect(check({ status: "on-break", activeReasonId: "lunch" })).toEqual([]);
    expect(check({ status: "on-break", reasons, activeReasonId: "tea" })).toEqual(["break.activeReasonId.known"]);
...
describe("rules that had no test", () => {
  ...
  it("event timestamps, outcomes, and status messages", () => {
    ...
    // An offer that lapsed at its assignment deadline was cancelled by nobody: it expired, pending.
    expect(ended({ type: "expired", phase: "pending" })).toEqual([]);
...
describe("the validators accept exactly what the contract publishes", () => {
  ...
  it("every published break kind, and no unpublished one", () => {
    const withKind = (kind: unknown) => snapshot({ break: { status: "not-requested", canRequestBreak: true, reasons: [{ id: "r1", label: "Rest", kind }] } });
...
describe("a transfer is not a command", () => {
  const voice = { channel: "voice" };
  const since = "2026-08-21T09:05:00Z";
  it("offers no transfer control, cold or warm, and knows nobody consulted on a call", () => {
    // Help arrives by lead assist; the agent hands the call to nobody. The former controls are
    // unknown capabilities, the former role is not one, and a conference still dials a colleague in.
    for (const former of ["coldTransfer", "warmTransfer"]) {
      expect(rules(validateTask(task({ capabilities: { [former]: { destinations: [{ id: "tier2", label: "Tier 2" }] } } }), voice))).toEqual(["task.capability.unknown"]);
    }
    expect(rules(validateTask(task({ capabilities: { conference: { destinations: [{ id: "tier2", label: "Tier 2" }] } } }), { ...voice, dialOutcomesDeclared: true }))).toEqual([]);
    const conferenced = { role: "conferenced", destinationId: "tier2", label: "Tier 2", dialId: "dial-3", stage: "ringing", since };
    expect(rules(validateTask(task({ onCall: [conferenced] }), voice))).toEqual([]);
    expect(rules(validateTask(task({ onCall: [{ ...conferenced, role: "consulted" }] }), voice))).toEqual(["task.onCall.role"]);
  ...
  it("keeps the record of a transfer the platform made: the step and the total stay", () => {
    // A call changes hands on the platform's own routing; the record says so, and the agent commanded none of it.
    expect(rules(validateTask(task({ history: { steps: [{ step: "transferred", at: since, destinationId: "tier2" }], transfers: 1 } }), voice))).toEqual([]);
    expect(rules(validateTaskCommand({ type: "transfer", action: "cold", dialId: "dial-2", destinationId: "tier2" }))).toEqual(["command.type"]);
    expect(rules(validateTaskCommand({ type: "transfer", action: "complete" }))).toEqual(["command.type"]);
  });
});
...
describe("who is on the call", () => {
  ...
  it("states each role with what that role needs, and refuses what another role would carry", () => {
    ...
    // Two conferenced people are ordinary; the singular rule is the party.
...
describe("every dial has an outcome", () => {
  ...
  it("requires a task that may dial to come from a manifest that says how a dial ends", () => {
    for (const name of ["connectBack", "conference"]) {
  ...
  it("carries the outcome once the manifest declared it, against a task or none, with the switch's words", () => {
    ...
    expect(check({ ...answered, assignmentId: "alloc-91", destinationId: "tier2" })).toEqual([]);
    // The outcome names the task by its assignment, so a late one lands on the assignment it names; the retired name is refused.
    expect(check({ ...answered, assignmentId: "alloc-91" })).toEqual([]);
    expect(check({ ...answered, assignmentId: "" })).toEqual(["event.dialOutcome.assignmentId"]);
    expect(check({ ...answered, assignmentId: "alloc-91", taskId: "call-91" })).toEqual(["event.assignmentId.renamed"]);
  ...
  it("lets a step that dialled say which dial and where, and no other step", () => {
    const step = (entry: Record<string, unknown>) => rules(validateTask(task({ history: { steps: [{ at: "2026-08-21T09:00:00Z", by: "A-1", ...entry }] } }), voice));
    ...
    expect(step({ step: "transferred", dialId: "" })).toEqual(["task.history.dialId"]);
    expect(step({ step: "transferred", destinationId: "" })).toEqual(["task.history.destinationId"]);
    expect(step({ step: "held", seconds: 12, dialId: "dial-7f2" })).toEqual(["task.history.dialId.unexpected"]);
    expect(step({ step: "answered", destinationId: "tier2" })).toEqual(["task.history.destinationId.unexpected"]);
  });
});
...
describe("consulting a lead", () => {
  ...
  it("carries a member's request on the member, naming the assignment the lead would join", () => {
    const since = "2026-08-21T09:04:00Z";
    const request = { assignmentId: "alloc-42", note: "Refund dispute", since };
    const trimmed = { assignmentId: "alloc-42", title: "Billing call", channel: "voice", taskType: "Billing", phase: "in-progress" };
    const asking = (over: Record<string, unknown> = {}) => [{ id: "A-1", availability: "on-task", request: { ...request, ...over } }];
    const lead = { capabilities: { lead: true as const } };
    const agent = { capabilities: {} };
    expect(rules(validateTeamMembers({ members: asking() }, "team", lead))).toEqual([]);
    // Nobody asking is absence: no request on the member, and no list to keep empty.
    expect(rules(validateTeamMembers({ members: [{ id: "A-1", availability: "on-task" }] }, "team", lead))).toEqual([]);
    // The login is the permission: nothing of the team reaches an agent who does not lead.
    expect(rules(validateTeamMembers({ members: asking() }, "team", agent))).toEqual(["team.unentitled"]);
    // Through the snapshot, the path adapters actually take.
    expect(rules(validateSnapshot(snapshot({ team: { members: asking() } }), manifest(), "snapshot", { ...lead, leadFeatures: true }))).toEqual([]);
    expect(rules(validateSnapshot(snapshot({ team: { members: asking() } }), manifest(), "snapshot", agent))).toEqual(["team.unentitled"]);
    // The request names one of the member's calls, where the list shows them.
    expect(rules(validateTeamMembers({ members: [{ ...asking()[0], tasks: [trimmed] }] }, "team", lead))).toEqual([]);
    expect(rules(validateTeamMembers({ members: [{ ...asking({ assignmentId: "alloc-43" })[0], tasks: [trimmed] }] }, "team", lead))).toEqual(["team.member.request.assignment"]);
    expect(rules(validateTeamMembers({ members: asking({ assignmentId: "" }) }, "team", lead))).toEqual(["team.member.request.assignmentId"]);
    expect(rules(validateTeamMembers({ members: asking({ since: "now" }) }, "team", lead))).toEqual(["team.member.request.since"]);
    expect(rules(validateTeamMembers({ members: asking({ note: "" }) }, "team", lead))).toEqual(["team.member.request.note"]);
    expect(rules(validateTeamMembers({ members: [{ id: "A-1", availability: "on-task", request: "help" }] }, "team", lead))).toEqual(["team.member.request.shape"]);
    // The lead's join and decline answer the member's own request, and no other member's.
    const context = { authentication: { status: "authenticated", identity: { id: "L-9", displayName: "Lead", timeZone: "Asia/Kolkata" }, capabilities: { lead: true } }, transport: "active", team: { members: asking() } };
    expect(rules(validateTeamCommand({ command: { type: "join", memberId: "A-1" } }, context))).toEqual([]);
    expect(rules(validateTeamCommand({ command: { type: "decline", memberId: "A-1", assignmentId: "alloc-42" } }, context))).toEqual([]);
    expect(rules(validateTeamCommand({ command: { type: "join", memberId: "A-1" } }, { ...context, team: { members: [{ id: "A-1", availability: "on-task" }] } }))).toEqual(["team.command.request"]);
  });
...
describe("validateTaskCommand", () => {
  const voice = task({ capabilities: { hold: true, endCall: true, recording: { provider: { start: true } }, conference: { destinations: [{ id: "tier2", label: "Tier 2" }] }, schedule: true } });
  const since = "2026-08-21T09:05:00Z";
  ...
  it("checks a command's own shape with or without a task", () => {
    ...
    // A follow-up on the calendar: the time, and a note where the agent wrote one.
    expect(rules(validateTaskCommand({ type: "schedule", at: since }))).toEqual([]);
    expect(rules(validateTaskCommand({ type: "schedule", at: since, note: "Call back about the refund" }))).toEqual([]);
    expect(rules(validateTaskCommand({ type: "schedule" }))).toEqual(["command.schedule.at"]);
  ...
  it("holds a destination to the directory the task offered", () => {
    ...
    const billing = task({ capabilities: { conference: { destinations: [{ id: "billing", label: "Billing" }] } } });
    expect(cmd({ type: "conference", action: "add", dialId: "dial-2", destinationId: "billing" }, billing)).toEqual([]);
    expect(cmd({ type: "conference", action: "add", dialId: "dial-2", destinationId: "tier2" }, billing)).toEqual(["command.destination.unknown"]);
  });
  ...
  it("holds a control on the contact to a contact being handled: in-progress or paused", () => {
    ...
    // A follow-up is promised on the call and written up in wrap alike: schedule reaches into completing.
    for (const phase of ["in-progress", "paused", "completing"]) {
      expect(cmd({ type: "schedule", at: since }, task({ ...voice, phase, ...(phase === "completing" ? { onCall: [] } : {}) })), phase).toEqual([]);
    }
    for (const phase of ["pending", "confirmed", "preview"]) {
      expect(cmd({ type: "schedule", at: since }, task({ ...voice, phase, ...(phase === "pending" ? { acceptance: "consent" } : {}) })), phase).toEqual(["command.phase.interaction"]);
    }
    expect(cmd({ type: "schedule", at: since }, task({ capabilities: { hold: true } }))).toEqual(["command.capability.schedule"]);
    expect(cmd({ type: "schedule", at: since }, task({ capabilities: { schedule: { lockedBy: "team" } } }))).toEqual(["command.capability.locked"]);
...
describe("preview: the agent presses Call", () => {
  ...
  it("carries the deadline and what happens at it together, and only while previewing", () => {
    ...
    // Seconds left from this publication, whole and not negative: never an instant to compare clocks against.
    for (const bad of ["soon", at, 1.5, -1]) expect(rules(validateTask(preview({ previewEndsInSeconds: bad, atDeadline: "provider-dials" }), voice))).toEqual(["task.preview.previewEndsInSeconds"]);
    expect(rules(validateTask(preview({ previewEndsInSeconds: 120, atDeadline: "dials" }), voice))).toEqual(["task.preview.atDeadline"]);
    expect(rules(validateTask(task({ phase: "in-progress", previewEndsInSeconds: 120, atDeadline: "provider-dials" }), voice))).toEqual(["task.preview.deadline.unexpected"]);
...
describe("the lead surface", () => {
  ...
  it("publishes the team while the feature is on, and nothing of it once the lead turned it off", () => {
    ...
    expect(rules(validateSnapshot(snapshot({ team }), manifest(), "snapshot", on))).toEqual([]);
    expect(rules(validateSnapshot(snapshot(), manifest(), "snapshot", on))).toEqual(["team.required"]);
    // Not yet told: the provider assumes nothing, and a reader who does not know checks nothing.
    expect(rules(validateSnapshot(snapshot(), manifest(), "snapshot", lead))).toEqual([]);
    ...
    // The per-member events are held to the same login and the same switch.
    const member = (context: unknown) => rules(validateEventEnvelope(envelope({ type: "team-member-updated", member: { id: "A-2", availability: "ready" } }), manifest(), "event", context as never));
    expect(member(on)).toEqual([]);
    expect(member(off)).toEqual(["event.team.features"]);
    expect(member({ capabilities: {} })).toEqual(["event.team.capability"]);
    const removed = (context: unknown) => rules(validateEventEnvelope(envelope({ type: "team-member-removed", memberId: "A-2" }), manifest(), "event", context as never));
    expect(removed(on)).toEqual([]);
    expect(removed(off)).toEqual(["event.team.features"]);
    expect(removed({ ...on, self: "A-2" })).toEqual(["team.member.self"]);
  });
  ...
  it("has no team audio: the lead's audio follows listening on the member, and the former events are unknown", () => {
    ...
    const on = (members: unknown[]) => rules(validateTeamMembers({ members }, "team", lead));
    // The provider's word that the lead's audio is up is `listening` on the member, naming the call to open.
    expect(on([{ id: "A-2", availability: "on-task", listening: [{ leadId: "L-9", assignmentId: "alloc-42", mode: "listen", since: at }] }])).toEqual([]);
...
describe("history and assignment API migration", () => {
  it("rejects former task history fields instead of silently ignoring their facts", () => {
    const history = { steps: [], interactionSeconds: 12 };
    expect(validateTask(task({ history: history }), { channel: "voice" })).toEqual([]);
    for (const former of ["handlingHistory", "interactionHistory"]) {
      for (const value of [undefined, history]) {
        expect(rules(validateTask(task({ [former]: value, history }), { channel: "voice" })), former)
          .toContain("task.history.renamed");
      }
    }
    expect(rules(validateTask(task({ history: { ...history, handleSeconds: 12 } }), { channel: "voice" })))
      .toContain("task.history.interactionSeconds.renamed");
  });

  it("rejects a task naming its life as an allocation, beside one naming it as an assignment", () => {
    const named = task({});
    expect(rules(validateTask(named, { channel: "voice" }))).toEqual([]);
    // renamed away: the offer's life is an assignment, and the old word is refused even beside the new one.
    expect(rules(validateTask({ ...named, allocationId: named.assignmentId }, { channel: "voice" })))
      .toEqual(["task.assignmentId.renamed"]);
    const { assignmentId, ...unnamed } = named;
    expect(rules(validateTask({ ...unnamed, allocationId: assignmentId }, { channel: "voice" })))
      .toEqual(["task.assignmentId.renamed", "task.assignmentId"]);
  });
