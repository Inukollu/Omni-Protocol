...
const task: Task<"voice"> = {
  id: "H2", assignmentId: "H2", title: "Interaction", channel: "voice", taskType: "Queue",
  phase: "in-progress", capabilitySource: "queue", capabilities: { endCall: true },
  browsers: [], completionMode: "agent-command",
};
const request = { taskId: "H2", assignmentId: "H2", command: { type: "end-call" } };
describe("provider interaction controls", () => {
  ...
  it("ends an old wrap without ending the newer interaction, and refuses the late old ending", () => {
    ...
    stream.resync({ tasks: [{ ...task, id: "H1", assignmentId: "H1", phase: "completing" }, task] }, "snapshot");
    const oldEnd = { event: { type: "task-ended", taskId: "H1", assignmentId: "H1", outcome: { type: "completed", by: "agent" } } };
    ...
    expect(stream.apply({ event: { type: "task-ended", taskId: "H2", assignmentId: "H1", outcome: { type: "completed", by: "agent" } } })).not.toEqual([]);
    expect(stream.apply({ event: { type: "task-updated", task } })).toEqual([]);
  });
  it("requires media to end before the owned interaction ends, including inherited channels", () => {
    ...
    expect(stream.apply({ event: { type: "task-media-ended", taskId: "H2", assignmentId: "H2" } })).toEqual([]);
    expect(stream.apply({ event: { type: "task-updated", task: { ...task, phase: "completing", media: "ended", onCall: [] } } })).toEqual([]);
    expect(stream.apply({ event: { type: "task-ended", taskId: "H2", assignmentId: "H2", outcome: { type: "completed", by: "agent" } } })).toEqual([]);
  });
});
